...
@Component({
  ...
  templateUrl: './applyuserloans.component.html',
  styleUrls: ['./applyuserloans.component.css'],
...
export class ApplyuserloansComponent extends BaseComponent implements OnInit {
  ...
  msgError: string;
  public defaultModeOfTransfer: { Mode: string, ID: number } = { Mode: "", ID: 4 };
  ...
  public GetLoansForUser(DCIDorName) {
    this.sharedService.setLoader(true);
    this.loanEarnsService._getLoansForUser(DCIDorName).subscribe((res: any) => {
      this.sharedService.setLoader(false);
      if (res.m_Item1) {
        this.listOfEarnLoans = [];
        this.listOfEarnLoans = res.m_Item3;
        this.appliedUserID = res.m_Item4;
  ...
  public checkRenewalStatus() {
    this.loanEarnsService._checkRenewalStatusByUserID(this.appliedUserID).subscribe((res: any) => {
  ...
  ApplyPersonalLoan(rowIndex) {
    try {
      this.loanEarnsService._checkPLExistByUserID(this.appliedUserID).subscribe((res: any) => {
        if (res.m_Item1) {
  ...
  onApplyPLLoan(formApplyPLLoan: any, isValidForm) {
    if (isValidForm) {
      this.apiManager.postAPI(API.PERSONALLOANBYADMIN, formApplyPLLoan).subscribe(response => {
        if (response.m_Item1) {
          this.isShowModal = 1;
  ...
  ApplyFeesReimbursement(rowIndex) {
    try {
      this.loanEarnsService._checkFRExistByUserID(this.appliedUserID).subscribe((res: any) => {
        if (res.m_Item1) {
  ...
  onApplyFeeReimbursement(formApplyFeeReimbursement: any, isValidForm) {
    if (isValidForm) {
      this.apiManager.postAPI(API.FEEREIMBURSEMENTBYADMIN, formApplyFeeReimbursement, this.myFiles).subscribe(response => {
        if (response.m_Item1) {
          this.isShowModal = 1;
          this.toastr.success(response.m_Item2);
  ...
  ApplyHealthBenefit(rowIndex) {
    try {
      this.loanEarnsService._checkHBExistByUserID(this.appliedUserID).subscribe((res: any) => {
        if (res.m_Item1) {
          this.isShowModal = 4;
          this.createApplyHealthBenefitsForm();
          this.selectedRow = this.listOfEarnLoans[rowIndex];
  ...
  onApplyHealthBenefits(formApplyHealthBenefits: any, isValidForm) {
    if (isValidForm) {
      this.apiManager.postAPI(API.HEALTHBENEFITSBYADMIN, formApplyHealthBenefits, this.myFiles).subscribe(response => {
        if (response.m_Item1) {
          this.isShowModal = 1;
          this.toastr.success(response.m_Item2);
  ...
  ApplyRiskBenefitLoan(rowIndex) {
    try {
      this.loanEarnsService._checkRBExistByUserID(this.appliedUserID).subscribe((res: any) => {
        if (res.m_Item1) {
          this.isShowModal = 5;
          this.createApplyRiskBenefitForm();
          this.selectedRow = this.listOfEarnLoans[rowIndex];
  ...
  onApplyRiskBenefit(formApplyRiskBenefit: any, isValidForm) {
    if (isValidForm) {
      this.apiManager.postAPI(API.RISKBENEFITSBYADMIN, formApplyRiskBenefit, this.myFiles).subscribe(response => {
        if (response.m_Item1) {
          this.isShowModal = 1;
          this.toastr.success(response.m_Item2);
  ...
  onApplyHomeLoan(formApplyHomeLoan: any, isValidForm) {
    if (isValidForm) {
      this.apiManager.postAPI(API.HOMELOANBYADMIN, formApplyHomeLoan).subscribe(response => {
        if (response.m_Item1) {
          this.isShowModal = 1;
          this.toastr.success(response.m_Item2);
  ...
  onSearchChange(searchValue: string) {
    if (!!searchValue && searchValue.trim().length >= 5) {
      this.GetLoansForUser(searchValue);
    }
  }
